...
export async function handleApiRequest(request: Request, url: URL, db: DB): Promise<Response> {
  ...
  if (method === 'PATCH' && singleMatch) {
    const body = await request.json<{ title?: string; notes?: string; due_date?: string; recurrence?: string; kickoff_note?: string; status?: 'pending' | 'snoozed'; snoozed_until?: string; focused_until?: string }>();
