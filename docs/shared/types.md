...
**`Task`** — Core task entity. Fields: `id`, `title`, `notes`, `status` (`pending | active | done | snoozed`), `due_date`, `recurrence`, `created_at`, `updated_at`, `snoozed_until`, `task_type` (`action | plan`), `project_id`, `kickoff_note`, `session_log`.
