...
**`syncFromServer(config, dispatch)`** — Fetches the full task, project, and link lists from the server, clears the corresponding IndexedDB stores, writes the server records in, and dispatches `SET_TASKS` / `SET_PROJECTS` / `SET_LINKS` to refresh React state. Returns a `SyncResult` indicating online status and the fetched data.
