...
Read tools (`list_tasks`, `get_ready_tasks`, `get_project_context`, `show_tasks`, `show_project`, `start_session`, `list_duties`) and `complete_task` call `materializeDueDuties` first so any duties whose `next_fire_at` has passed are turned into real tasks before the response goes out. Legacy recurring tasks are converted during that materialization only after an explicit valid timezone preference exists.
