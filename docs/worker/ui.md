...
- `GET /ui/active` — Verifies the request signature, then returns the active-tasks widget HTML (from `app-ui.ts`). The widget communicates back to the worker via `postMessage` JSON-RPC for real-time updates without a page reload.
