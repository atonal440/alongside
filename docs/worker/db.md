...
**`focusTask(id, focusedUntil)`** — Loads the row as `PendingTaskDomain`, parses the focus timestamp, and plans the focus transition. Deferred tasks return an `invalid_transition` error.
