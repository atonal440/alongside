...
- `GET /oauth/authorize` — Authorization endpoint. Validates `code_challenge` (PKCE), stores a short-lived 32-character authorization code in D1, and redirects to the client's `redirect_uri` with the code.
...
- [[index|worker/src/index.ts]] — routes `/oauth/*` requests to `handleOAuthRequest`
- [[schema]] — `oauthCodes` table that stores short-lived authorization codes
- [[mcp-tools]] — MCP tools that callers access after completing the OAuth flow
