...
**`unlinkTasksPlan(link)`** — Emits one `link.delete` op. It intentionally does not assert endpoint existence or reject self-links, preserving the previous no-op behavior for deleting absent links and allowing cleanup of legacy bad rows.
