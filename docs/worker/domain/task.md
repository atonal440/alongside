...
**`recurrenceFromRow(dueDate, recurrence)`** — Parses the row-shaped recurrence fields into the `Recurrence` union. It validates `due_date` when present, parses the RRULE into branded `Rrule` plus `RruleParts`, rejects `recurrence != null` without a due date, and verifies the rule can produce a next occurrence after that due date.
