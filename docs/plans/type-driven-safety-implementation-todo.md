...
- [x] When the type-driven safety work reaches the frontend, distinguish durable 4xx validation failures from transient offline/network failures in the PWA sync layer. Today `apiFetch` collapses all non-OK responses to `null`, so rejected writes can be queued like offline retries; the frontend migration should parse validation errors, surface them to the user, and avoid retry loops. → **Resolved in PWA stage 5**: `ApiResult` discriminant + `isDurableFailure`/`isTransientFailure` from `result.ts`; 4xx drops + toasts, transient queues.
- [x] Link mutations need the same treatment specifically: `createLinkAction` writes optimistically to IndexedDB before the server can reject self-links, missing endpoints, or `blocks` cycles. The PWA pass should roll back or annotate durable link rejections instead of leaving invalid local graph state and retrying the rejected op forever. → **Resolved in PWA stage 5**: durable link rejections toast + trigger resync; `listLinks` from server restores the correct graph. Nothing is queued for retry on 4xx.
